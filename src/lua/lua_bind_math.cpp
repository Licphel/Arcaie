...
void lua_bind_math(lua_state &lua)
{
    ...
    vec2_type["__add"] = lua_combine(P_TYPE_OP_C(vec2, vec2, const vec2 &, +), P_TYPE_OP_C(vec2, vec2, double, +));
    vec2_type["__sub"] = lua_combine(P_TYPE_OP_C(vec2, vec2, const vec2 &, -), P_TYPE_OP_C(vec2, vec2, double, -));
    vec2_type["__mul"] = lua_combine(P_TYPE_OP_C(vec2, vec2, const vec2 &, *), P_TYPE_OP_C(vec2, vec2, double, *));
    vec2_type["__div"] = lua_combine(P_TYPE_OP_C(vec2, vec2, const vec2 &, /), P_TYPE_OP_C(vec2, vec2, double, /));
    ...
    vec3_type["__add"] = lua_combine(P_TYPE_OP_C(vec3, vec3, const vec3 &, +), P_TYPE_OP_C(vec3, vec3, double, +));
    vec3_type["__sub"] = lua_combine(P_TYPE_OP_C(vec3, vec3, const vec3 &, -), P_TYPE_OP_C(vec3, vec3, double, -));
    vec3_type["__mul"] = lua_combine(P_TYPE_OP_C(vec3, vec3, const vec3 &, *), P_TYPE_OP_C(vec3, vec3, double, *));
    vec3_type["__div"] = lua_combine(P_TYPE_OP_C(vec3, vec3, const vec3 &, /), P_TYPE_OP_C(vec3, vec3, double, /));
    ...
    trs_type["apply"] = &transform::apply;
