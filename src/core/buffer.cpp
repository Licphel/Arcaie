...
void byte_buf::ensure_readable(size_t needed) const
{
    if (readable_bytes() < needed)
        prtlog_throw(ARC_FATAL, "byte buffer read out of range!");
}
...
{
    if (pos > __wpos)
        prtlog_throw(ARC_FATAL, "byte buf read pos out of range!");
    __rpos = pos;
}
...
std::vector<byte> byte_buf::to_vector() const
{
    if (__wpos > __data.size())
        prtlog_throw(ARC_FATAL, "byte buf write pos out of range!");
